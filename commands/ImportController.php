...
class ImportController extends Controller{
    ...
    public function importProduct($params){
        ...
        if ($cat = Category::find()->where(['id' => $params['idCategory']])->one()){
            if ($cat->isWork){
                ...
                // echo "import1C: ".$params['import1C']."\n";
                if ($product = Product::find()->where(['id1C' => $params['idProduct']])->one()){
                   // if ($product->import1C == $params['import1C']){
                   //     return true;
                   // }
                   // echo "change: ".$params['import1C']."\n";
                }else{
                    $product = new Product();
                ...
                // $product->import1C = $params['import1C'];
    ...
    public function getImportFilename($i){
        $filename = \Yii::$app->params['dirImport'].'/1c_'.$i.'.txt';
        echo $filename."\n";
        return $filename;
    }
    ...
    public function import0(){
        if (file_exists($this->getImportFilename(0))){            
            if ($this->f = fopen($this->getImportFilename(0), "r")){
                echo "in import0\n";
                ...
                $this->importCategory();
                fclose($this->f);
                Product::updateAll(['isImport' => 0]);
            }
        }
    }


    public function importProductN($i){
        echo " **** in importProductN() ".$i."\n";
        $this->f = fopen($this->getImportFilename($i), "r");
        if ($this->f){            
    ...
    public function importClean($isFirst = 0){
        if ($isFirst){
        ...
        foreach ($list as $product){
            if ($cat = Category::findOne($product->idCategory)){
                if ($cat->isWork == 0){
                    echo "delete: ".$product->name."\n";
                    // \Yii::$app
                    //     ->db
                    //     ->createCommand()
                    //     ->delete('product', ['id' => $product->id])
                    //     ->execute();
                    echo Product::findOne($product->id)->delete();
                    //echo $product->delete()."\n";
                }
            }
            $product->isHandled = 1;
     ...
     *      - 'delZero1C'
     */
    public function actionIndex($mode, $nImport=0){
        $idCategory = 0;
        switch ($mode){
            case 'importCleanStart':
                $this->importClean(1);
                break;
            case 'importClean':
                $this->importClean();
